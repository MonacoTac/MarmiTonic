...
class TestSparqlService:
    ...
    def test_get_all_cocktails_from_dbpedia_error(self, sparql_service):
        import requests
        with patch('services.sparql_service.SPARQLWrapper.query', side_effect=requests.ConnectionError()), \
             patch('services.sparql_service.requests.get', side_effect=requests.ConnectionError()):
            result = sparql_service.get_all_cocktails_from_dbpedia()
            assert result is None
