...
class TestGraphService:
    ...
    def test_get_graph_data_success(self, graph_service):
        mock_data = {
            "results": {
                "bindings": [
                    {"cocktail": {"value": "http://example.com/cocktail1"}, "ingredient": {"value": "http://example.com/ingredient1"}},
                    {"cocktail": {"value": "http://example.com/cocktail2"}, "ingredient": {"value": "http://example.com/ingredient2"}}
                ]
            }
        }

        # Mock the query_local_data method to return our test data
        graph_service.sparql_service.query_local_data = lambda query: mock_data
    ...
    def test_get_graph_data_empty(self, graph_service):
        # Mock the query_local_data method to return empty data
        graph_service.sparql_service.query_local_data = lambda query: {"results": {"bindings": []}}
